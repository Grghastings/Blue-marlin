...
function filterData(){
  ...
  return DATA.filter(r=>{
    const hay=[r.bureau,r.status,r.title,r.awardId,r.winner,r.why,r.instrument].join(' ').toLowerCase();
    const amount=val(r);
    return (!q||hay.includes(q))&&(!bureau||r.bureau===bureau)&&(!status||r.status===status)&&(!rel||r.relevance===rel)&&(!range||(amount>=lo&&amount<=hi));
  }).sort((a,b)=>{
...
function render(){const rows=filterData();renderKpis(rows);renderBars(rows,'status','#statusBars');renderBars(rows,'bureau','#bureauBars');renderRows(rows)}
['#search','#bureau','#status','#relevance','#valueRange'].forEach(id=>$(id).addEventListener(id==='#search'?'input':'change',render));
$('#reset').addEventListener('click',()=>{['#search','#bureau','#status','#relevance','#valueRange'].forEach(id=>$(id).value='');render()});
